...
function AuthBadge({
	...
	return (
		...
		>
			{t("authAuthtext", "auth: {{authText}}", { authText })}
		</Badge>
	);
}
	...
	return (
		<div className="w-full space-y-4">
			{shows("server") && (
				<>
					<Alert>
						...
						<AlertDescription>
							{t(
								"spinsUpARemoteRestApiServerEndpointsMethodsAuthenticationAndRequestSchemasAreDeclaredInsideTheWorkflowBoardTheServerIsMountedAt",
								"Spins up a remote REST API server. Endpoints, methods, authentication and request schemas are declared inside the workflow board. The server is mounted at",
							)}{" "}
			...
			<p className="text-xs text-muted-foreground flex items-center gap-1">
				...
				{t(
					"saveTheEventToTriggerARemoteSetupThatRegistersAllDeclaredRoutes",
					"Save the event to trigger a remote setup that registers all declared routes.",
				)}
			</p>
		</div>
	);
			...
			<div className="grid gap-2 lg:grid-cols-3">
				<div className="space-y-1 rounded-md border bg-background p-2">
					...
					<div className="font-mono text-xs">
						{version ?? t("notRegistered", "not registered")}
					</div>
				</div>
				<div className="space-y-1 rounded-md border bg-background p-2">
					<div className="text-[10px] uppercase text-muted-foreground">
				...
				<div className="space-y-1 rounded-md border bg-background p-2">
					...
					<div className="text-xs">
						{loading
							? "loading"
							: t("lengthRegistered", "{{length}} registered", {
					...
					{missingAuthIds.map((authId) => (
						...
						>
							<span className="text-muted-foreground">
								{t(
									"authIsLinkedToRoutesButDetailsAreNotAvailable",
