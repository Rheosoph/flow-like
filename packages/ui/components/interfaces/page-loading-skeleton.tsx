...
const PHASE_IDS = ["initializing", "resources", "processing", "interface"];
...
function PhaseReadout({ title }: Readonly<{ title: string }>) {
	...
	const phases = [
		t("initializingWorkflow", "Initializing workflow"),
		t("loadingResources", "Loading resources"),
		t("processingData", "Processing data"),
		t("preparingInterface", "Preparing interface"),
	];
	...
	useEffect(() => {
		...
		const id = setInterval(() => {
			...
			const next = (previous + 1) % PHASE_IDS.length;
...
export function PageLoadingSkeleton({
	...
	return (
		...
		>
			<LatticeCanvas />
			<PhaseReadout title={title ?? t("runningWorkflow", "Running workflow")} />
