...
export function QueryResultChart({
	...
	const summary = t(
		"typeChartOfValByVal2",
		"{{type}} chart of {{val}} by {{val2}}",
		{
			type: config.type,
			val: yKeys.map(humanizeIdentifier).join(", "),
			val2: humanizeIdentifier(xKey),
		},
	);
		...
		<div className="relative flex h-full min-h-0 flex-col">
					...
					<PopoverContent align="end" className="w-72 space-y-3">
						<div className="grid gap-1.5">
							...
							>
								<SelectTrigger
									className="h-8"
									aria-label={t("chartType", "Chart type")}
								>
									<SelectValue />
								</SelectTrigger>
								<SelectContent>
			...
			{!canRender ? (
				<div className="flex h-full items-center justify-center rounded-lg border border-dashed text-sm text-muted-foreground">
					{t(
						"pickACategoryAndAtLeastOneValueColumnToChart",
