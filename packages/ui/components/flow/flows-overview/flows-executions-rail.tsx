				...
				{buckets.map(({ hoursAgo, total, failed }) => {
					...
					return (
						<div
							...
							title={`${hoursAgo}–${hoursAgo - 2} h ago: ${total} ${total === 1 ? "run" : "runs"}, ${failed} failed`}
						>
							{total === 0 ? (
	...
	return (
		...
		>
							...
							<div key={bucket}>
								...
								{items.map((run) => (
									<RunRow
										...
										run={run}
										now={now}
										eventName={eventNames.get(run.eventId) ?? "Direct run"}
			...
			<p className="border-t border-border/50 pt-2 text-[10px] leading-relaxed text-muted-foreground/60">
				Read from the run log — up to 200 runs each across at most 25 flows,
				refreshed every minute. Outcome comes from each run&rsquo;s log level;
