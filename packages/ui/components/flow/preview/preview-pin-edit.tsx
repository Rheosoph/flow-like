			...
			<HoverCard openDelay={0} closeDelay={0}>
				<HoverCardTrigger>
					<Button
						size={"icon"}
						variant={"ghost"}
						className="w-fit h-fit text-foreground bg-background p-0"
					>
						<Dialog>
							<DialogTrigger>
								<VariableIcon
									className={`w-[0.45rem] h-[0.45rem] min-w-[0.45rem] min-h-[0.45rem] p-0 bg-background ${(typeof defaultValue === "undefined" || defaultValue === null) && "text-primary"}`}
								/>
							</DialogTrigger>
							<DialogContent>
								<DialogHeader>
									<DialogTitle>
										{t("setDefaultValue", "Set Default Value")}
									</DialogTitle>
									<DialogDescription>
										{t(
											"theDefaultValueWillOnlyBeUsedIfThePinIsNotConnected",
