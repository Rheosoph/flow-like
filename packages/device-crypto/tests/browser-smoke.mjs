...
import { webcrypto, createHash, createPrivateKey, sign, X509Certificate } from "node:crypto";
...
// Public PKCS#10 fixtures generated by OpenSSL. Their private keys were discarded.
const serviceCsr = `-----BEGIN CERTIFICATE REQUEST-----

AIl+YfirFAPqmSvvmKWItsTMFIGWIV36AJORwQxzM8EV
-----END CERTIFICATE REQUEST-----`;
const issuerCsr = `-----BEGIN CERTIFICATE REQUEST-----

MeQvD8n3
-----END CERTIFICATE REQUEST-----`;

test("generated browser WASM keeps CA keys in scoped vaults and signs only approved service CSRs", () => {
  const password = new TextEncoder().encode("organisation CA browser password");
  const replacement = new TextEncoder().encode("replacement organisation password");
  const account = JSON.stringify(["https://auth.example", "owner", "https://hub.example", "profile"]);
  const authorityId = "bce0a1fe-7055-4131-8e08-ec9b3b4b3975";
  const now = 1_790_000_000;
  try {
    const created = crypto.createCertificateAuthorityVault({
      account_binding: account, authority_id: authorityId, label: "Browser organisation",
      dns_suffixes: ["example.test"], ip_addresses: ["127.0.0.1"], validity_days: 3650,
    }, password, now);
    const imported = JSON.parse(JSON.stringify(created));
    assert.equal(JSON.stringify(imported).includes("PRIVATE KEY"), false);
    const vault = Uint8Array.from(imported.vault);
    const rootVault = Uint8Array.from(imported.root_vault);
    assert.deepEqual(crypto.inspectCertificateAuthorityVault(account, authorityId, password, vault), created.public_bundle);
    assert.deepEqual(crypto.inspectCertificateAuthorityBackup(account, authorityId, password, vault, rootVault), created.public_bundle);
    const damagedRoot = rootVault.slice();
    damagedRoot[damagedRoot.length - 1] ^= 1;
    assert.throws(() => crypto.inspectCertificateAuthorityBackup(account, authorityId, password, vault, damagedRoot));
    assert.throws(() => crypto.inspectCertificateAuthorityBackup(account, authorityId, password, vault, vault));
    assert.throws(() => crypto.inspectCertificateAuthorityVault("different account", authorityId, password, vault));
    assert.throws(() => crypto.inspectCertificateAuthorityVault(account, authorityId, replacement, vault));
    assert.throws(() => crypto.inspectCertificateAuthorityVault(account, authorityId, password, rootVault));
    const request = { csr_pem: serviceCsr, dns_names: ["api.example.test"], ip_addresses: ["127.0.0.1"], validity_days: 90 };
    const signed = crypto.signServiceCertificate(account, authorityId, password, vault, request, now);
    const leaf = new X509Certificate(signed.certificate_chain_pem);
    const intermediate = new X509Certificate(created.public_bundle.issuer_certificate_pem);
    const root = new X509Certificate(created.public_bundle.root_certificate_pem);
    assert.equal(root.ca, true);
    assert.equal(leaf.ca, false);
    assert.equal(leaf.checkHost("api.example.test"), "api.example.test");
    ...
    assert.equal(root.fingerprint256.replaceAll(":", "").toLowerCase(), created.public_bundle.sha256_fingerprint);
    assert.equal(signed.not_after, now + 90 * 86400);
    assert.throws(() => crypto.signServiceCertificate(account, authorityId, password, vault, { ...request, dns_names: ["other.example.test"] }, now));
    assert.throws(() => crypto.signServiceCertificate(account, authorityId, password, vault, { ...request, csr_pem: issuerCsr }, now));
    const delegated = crypto.signDeviceCertificateIssuer(account, authorityId, password, vault, { ...request, csr_pem: issuerCsr }, now);
    assert.equal(new X509Certificate(delegated.certificate_chain_pem).ca, true);
    assert.equal(new X509Certificate(delegated.certificate_chain_pem).verify(intermediate.publicKey), true);
    const renewed = crypto.renewCertificateAuthorityVault(account, authorityId, password, vault, rootVault, now + 86400);
    assert.equal(renewed.public_bundle.root_certificate_pem, created.public_bundle.root_certificate_pem);
    assert.notEqual(renewed.public_bundle.issuer_certificate_pem, created.public_bundle.issuer_certificate_pem);
    assert.throws(() => crypto.rewrapCertificateAuthorityVault(account, authorityId, password, replacement, Uint8Array.from(renewed.vault), rootVault));
    const rewrapped = crypto.rewrapCertificateAuthorityVault(account, authorityId, password, replacement, Uint8Array.from(renewed.vault), Uint8Array.from(renewed.root_vault));
    assert.deepEqual(crypto.inspectCertificateAuthorityVault(account, authorityId, replacement, Uint8Array.from(rewrapped.vault)), renewed.public_bundle);
    assert.throws(() => crypto.inspectCertificateAuthorityVault(account, authorityId, password, Uint8Array.from(rewrapped.vault)));
  } finally {
    password.fill(0);
    replacement.fill(0);
  }
