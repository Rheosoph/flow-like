...
function videoInfo(provider: VideoProvider, id: string): VideoInfo {
	return { provider, id, mediaType: "video" };
}

function parseYoutube(url: URL, host: string, path: string[]) {
	let id;
...
function parseVimeo(host: string, path: string[]) {
...
function parseDailymotion(host: string, path: string[]) {
	let candidate;
...
function parseYouku(host: string, path: string[]) {
...
function parseCoub(host: string, path: string[]) {
