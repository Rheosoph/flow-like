...
function parseYoutube(url, host, path) {
	...
	if (host === "youtu.be") {
		...
		id = validId(url.searchParams.get("v"), YOUTUBE_ID);
...
function parseDailymotion(host, path) {
	...
	return id ? { provider: "dailymotion", id } : undefined;
}
...
function parseYouku(host, path) {
	...
	if (host === "player.youku.com" && path[0] === "embed") {
		...
		const match = urlPath(path).match(/(?:^|\/)v_show\/id_([A-Za-z0-9=_-]{1,128})(?:\.html)?$/);
		candidate = match?.[1];
	}

	const id = validId(candidate, YOUKU_ID);
	return id ? { provider: "youku", id } : undefined;
}

function parseCoub(host, path) {
	if (!isHost(host, "coub.com")) return undefined;
	const [prefix, candidate] = path;
	const id = ["view", "embed"].includes(prefix) ? validId(candidate, COUB_ID) : undefined;

	return id ? { provider: "coub", id } : undefined;
}
