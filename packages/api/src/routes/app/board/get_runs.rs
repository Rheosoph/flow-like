...
use flow_like::flow::execution::run_index::summary_row;
...
use crate::{
    ...
    error::ApiError,
    execution::run_summary::{log_meta_from_run, without_nodes},
...
pub struct ListRunsQuery {
    ...
    /// Also return each run's per-node activity summary (visited nodes + max
    /// severity). Opt-in: clients request it only when rendering the activity
    /// heatmap, which keeps the plain listing light.
...
pub async fn get_runs(
    ...
    let include_nodes = query.include_nodes.unwrap_or(false);
    if !include_nodes {
        db_query = without_nodes(db_query);
    }
    ...
    let log_metas: Vec<LogMeta> = runs
        .iter()
        .map(|run| summary_row(&log_meta_from_run(run), include_nodes))
        .collect();
