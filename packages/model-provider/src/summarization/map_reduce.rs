...
pub async fn map_reduce_summarize(
    ...
    let summaries: Vec<String> = if effective_concurrency <= 1 {
        ...
        let tasks: Vec<_> = chunks
            ...
            .map(|chunk| {
                ...
                async move {
                    invoke_llm(model_ref, &mn, &sys, &prompt).await
                }
            })
            .collect();
    ...
    // --- Reduce phase ---
    let (result, reduce_calls) =
        recursive_reduce(&summaries, instructions, entity_context, model, model_name, chunk_capacity)
            .await?;
...
pub fn recursive_reduce<'a>(
    ...
    model_name: &'a str,
    chunk_capacity: usize,
) -> std::pin::Pin<Box<dyn std::future::Future<Output = flow_like_types::Result<(String, usize)>> + Send + 'a>> {
    Box::pin(async move {
    if summaries.len() <= 1 {
    ...
    if batch_results.len() <= 1 {
        return Ok((batch_results.into_iter().next().unwrap_or_default(), llm_calls));
    }

    let (final_result, nested_calls) =
        recursive_reduce(&batch_results, instructions, entity_context, model, model_name, chunk_capacity)
            .await?;
