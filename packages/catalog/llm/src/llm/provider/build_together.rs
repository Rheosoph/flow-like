...
#[crate::register_node]
#[derive(Default)]
pub struct BuildTogetherNode {}

impl BuildTogetherNode {
    pub fn new() -> Self {
        BuildTogetherNode {}
    }
}
...
#[async_trait]
impl NodeLogic for BuildTogetherNode {
    fn get_node(&self) -> Node {
        let mut node = Node::new(
            "ai_generative_build_together",
            "Together AI Model",
            "Builds the Together AI model based on certain selection criteria",
            "AI/Generative/Provider",
        );
        node.set_flowscript_name("ai.provider", "together");
        ...
        node.add_input_pin(
            "exec_in",
            "Input",
            "Execution trigger that builds or refreshes the Together Bit",
        ...
        node.add_input_pin(
            "endpoint",
            "Endpoint",
            "Together API endpoint or regional proxy",
            VariableType::String,
        )
        .set_default_value(Some(json!("https://api.together.xyz")));

        node.add_input_pin(
            "api_key",
            "API Key",
            "Token used for authenticating against Together",
            VariableType::String,
        )
        ...
        .set_default_value(Some(json!("meta-llama/Llama-3.3-70B-Instruct-Turbo")));
        ...
        node.add_output_pin(
            "model",
            "Model",
            "Structured Bit describing the Together provider",
            VariableType::Struct,
        )
        .set_schema::<Bit>();
    ...
    async fn run(&self, context: &mut ExecutionContext) -> flow_like_types::Result<()> {
        ...
        let params_obj = VLMParameters {
            ...
            provider: flow_like_model_provider::provider::ModelProvider {
                api_surface: None,
                provider_name: "custom:together".into(),
