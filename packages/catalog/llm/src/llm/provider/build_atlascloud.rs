...
#[crate::register_node]
#[derive(Default)]
pub struct BuildAtlasCloudNode {}

impl BuildAtlasCloudNode {
    pub fn new() -> Self {
        BuildAtlasCloudNode {}
    }
}
...
#[async_trait]
impl NodeLogic for BuildAtlasCloudNode {
    fn get_node(&self) -> Node {
        let mut node = Node::new(
            "ai_generative_build_atlascloud",
            "Atlas Cloud Model",
        ...
        node.add_input_pin(
            "exec_in",
            "Input",
            "Execution trigger that builds or refreshes the Atlas Cloud Bit",
        ...
        node.add_input_pin(
            "endpoint",
            "Endpoint",
            "Atlas Cloud OpenAI-compatible base URL (override only for a proxy)",
            VariableType::String,
        )
        .set_default_value(Some(json!("https://api.atlascloud.ai/v1")));

        node.add_input_pin(
            "api_key",
            "API Key",
            "Atlas Cloud API key used for authentication",
            VariableType::String,
        )
        ...
        node.add_input_pin(
            "model_id",
            "Model ID",
            "Atlas Cloud model identifier to request (e.g., deepseek-ai/deepseek-v4-pro)",
            VariableType::String,
        )
        .set_default_value(Some(json!("deepseek-ai/deepseek-v4-pro")));
        ...
        node.add_output_pin(
            "model",
            "Model",
            "Structured Bit describing the Atlas Cloud provider",
            VariableType::Struct,
        )
        .set_schema::<Bit>();
    ...
    async fn run(&self, context: &mut ExecutionContext) -> flow_like_types::Result<()> {
        ...
        // Atlas Cloud is OpenAI-compatible, so it reuses the existing
