...
#[crate::register_node]
#[derive(Default)]
pub struct BuildMistralNode {}

impl BuildMistralNode {
    pub fn new() -> Self {
        BuildMistralNode {}
    }
}
...
#[async_trait]
impl NodeLogic for BuildMistralNode {
    fn get_node(&self) -> Node {
        let mut node = Node::new(
            "ai_generative_build_mistral",
            "Mistral Model",
            "Builds the Mistral model based on certain selection criteria",
            "AI/Generative/Provider",
        );
        node.set_flowscript_name("ai.provider", "mistral");
    ...
    async fn run(&self, context: &mut ExecutionContext) -> flow_like_types::Result<()> {
        ...
        let params_obj = VLMParameters {
            ...
            provider: flow_like_model_provider::provider::ModelProvider {
                api_surface: None,
                provider_name: "custom:mistral".into(),
