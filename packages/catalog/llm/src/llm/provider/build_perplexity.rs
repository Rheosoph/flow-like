...
#[crate::register_node]
#[derive(Default)]
pub struct BuildPerplexityNode {}

impl BuildPerplexityNode {
    pub fn new() -> Self {
        BuildPerplexityNode {}
    }
}
...
#[async_trait]
impl NodeLogic for BuildPerplexityNode {
    fn get_node(&self) -> Node {
        let mut node = Node::new(
            "ai_generative_build_perplexity",
            "Perplexity Model",
            "Builds the Perplexity model based on certain selection criteria",
            "AI/Generative/Provider",
        );
        node.set_flowscript_name("ai.provider", "perplexity");
        ...
        node.add_input_pin(
            "exec_in",
            "Input",
            "Execution trigger that builds or refreshes the Perplexity Bit",
        ...
        node.add_input_pin(
            "endpoint",
            "Endpoint",
            "Perplexity API endpoint or self-hosted base URL",
            VariableType::String,
        )
        .set_default_value(Some(json!("https://api.perplexity.ai")));

        node.add_input_pin(
            "api_key",
            "API Key",
            "Token used for authenticating against Perplexity",
            VariableType::String,
        )
        ...
        node.add_input_pin(
            "model_id",
            "Model ID",
            "Model identifier or preset slug to request",
            VariableType::String,
        )
        .set_default_value(Some(json!("llama-3.1-sonar-small-128k-online")));
        ...
        node.add_output_pin(
            "model",
            "Model",
            "Structured Bit describing the Perplexity provider",
            VariableType::Struct,
        )
        .set_schema::<Bit>();
    ...
    async fn run(&self, context: &mut ExecutionContext) -> flow_like_types::Result<()> {
        ...
        let params_obj = VLMParameters {
            ...
            provider: flow_like_model_provider::provider::ModelProvider {
                api_surface: None,
                provider_name: "custom:perplexity".into(),
