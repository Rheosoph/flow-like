...
#[async_trait]
impl NodeLogic for BuildGroqNode {
    fn get_node(&self) -> Node {
        let mut node = Node::new(
            "ai_generative_build_groq",
            "Groq Model",
        ...
        .set_default_value(Some(json!("https://api.groq.com/openai/v1")));

        node.add_input_pin("api_key", "API Key", "Groq API key", VariableType::String)
        ...
        node.add_input_pin(
            "model_id",
            "Model ID",
            "Groq-served model identifier",
            VariableType::String,
        )
        .set_default_value(Some(json!("llama-3.3-70b-versatile")));
