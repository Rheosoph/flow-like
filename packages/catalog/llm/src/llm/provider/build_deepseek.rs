...
#[crate::register_node]
#[derive(Default)]
pub struct BuildDeepseekNode {}

impl BuildDeepseekNode {
    pub fn new() -> Self {
        BuildDeepseekNode {}
    }
}
...
#[async_trait]
impl NodeLogic for BuildDeepseekNode {
    fn get_node(&self) -> Node {
        let mut node = Node::new(
            "ai_generative_build_deepseek",
            "Deepseek Model",
            "Prepares a Bit for Deepseek's API using the provided credentials",
            "AI/Generative/Provider",
        );
        node.set_flowscript_name("ai.provider", "deepseek");
        ...
        .set_default_value(Some(json!("https://api.deepseek.com")));
        ...
        .set_default_value(Some(json!("deepseek-chat")));
    ...
    async fn run(&self, context: &mut ExecutionContext) -> flow_like_types::Result<()> {
        ...
        let params_obj = VLMParameters {
            ...
            provider: flow_like_model_provider::provider::ModelProvider {
                api_surface: None,
                provider_name: "custom:deepseek".into(),
