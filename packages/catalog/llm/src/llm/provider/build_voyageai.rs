...
#[crate::register_node]
#[derive(Default)]
pub struct BuildVoyageAINode {}

impl BuildVoyageAINode {
    pub fn new() -> Self {
        BuildVoyageAINode {}
    }
}
...
#[async_trait]
impl NodeLogic for BuildVoyageAINode {
    fn get_node(&self) -> Node {
        let mut node = Node::new(
            "ai_generative_build_voyageai",
            "VoyageAI Model",
        ...
        node.add_input_pin(
            "exec_in",
            "Input",
            "Execution trigger that builds or refreshes the VoyageAI Bit",
        ...
        node.add_input_pin(
            "endpoint",
            "Endpoint",
            "VoyageAI API base URL or custom proxy",
            VariableType::String,
        )
        .set_default_value(Some(json!("https://api.voyageai.com/v1")));

        node.add_input_pin(
            "api_key",
            "API Key",
            "Token used for authenticating against VoyageAI",
            VariableType::String,
        )
        ...
        node.add_input_pin(
            "model_id",
            "Model ID",
            "Model identifier or preset slug to use",
            VariableType::String,
        )
        .set_default_value(Some(json!("voyage-3")));
        ...
        node.add_output_pin(
            "model",
            "Model",
            "Structured Bit describing the VoyageAI provider",
            VariableType::Struct,
        )
        .set_schema::<Bit>();
    ...
    async fn run(&self, context: &mut ExecutionContext) -> flow_like_types::Result<()> {
        ...
        let params_obj = VLMParameters {
            ...
            provider: flow_like_model_provider::provider::ModelProvider {
                provider_name: "custom:voyageai".into(),
