...
#[crate::register_node]
#[derive(Default)]
pub struct BuildHyperbolicNode {}

impl BuildHyperbolicNode {
    pub fn new() -> Self {
        BuildHyperbolicNode {}
    }
}
...
#[async_trait]
impl NodeLogic for BuildHyperbolicNode {
    fn get_node(&self) -> Node {
        let mut node = Node::new(
            "ai_generative_build_hyperbolic",
            "Hyperbolic Model",
            "Builds the Hyperbolic model based on certain selection criteria",
            "AI/Generative/Provider",
        );
        node.set_flowscript_name("ai.provider", "hyperbolic");
    ...
    async fn run(&self, context: &mut ExecutionContext) -> flow_like_types::Result<()> {
        ...
        let params_obj = VLMParameters {
            ...
            provider: flow_like_model_provider::provider::ModelProvider {
                provider_name: "custom:hyperbolic".into(),
