...
use flow_like_types::json::Serialize;
...
impl Retarget<'_> {
    ...
    fn walk_embedded_json(&mut self, component_id: &str, raw: &mut String) {
        ...
        match flow_like_types::json::to_string(&parsed) {
    ...
    fn walk_json_blob(&mut self, component_id: &str, blob: &mut Vec<u8>) {
        let Ok(mut parsed) = flow_like_types::json::from_slice::<Value>(blob) else {
            return;
        };
        let before = self.changes.len();
        self.walk(component_id, &mut parsed);
        ...
        match flow_like_types::json::to_vec(&parsed) {
...
fn is_workflow_event_action(map: &flow_like_types::json::Map<String, Value>) -> bool {
