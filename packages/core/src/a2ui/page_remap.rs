...
use flow_like_types::{Value, proto};
...
fn remap_bound_value(field: &str, bound: &mut proto::BoundValue, translators: &mut IdTranslators) {
    ...
    match value {
        proto::bound_value::Value::LiteralString(literal) => {
            let mut wrapped = flow_like_types::json::Map::new();
...
fn remap_blob(bytes: &mut Vec<u8>, rewrite: impl FnOnce(&mut Value)) -> Result<(), String> {
    ...
    let mut value: Value =
        flow_like_types::json::from_slice(bytes).map_err(|err| format!("parse failed: {err}"))?;
    rewrite(&mut value);
    let encoded =
        flow_like_types::json::to_vec(&value).map_err(|err| format!("re-encode failed: {err}"))?;
    *bytes = encoded;
    Ok(())
}
