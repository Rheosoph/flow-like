...
export type ProbePhase = "suite" | "navigation";
...
const GRANT_IN_DOCUMENT = new RegExp(`(index\\.)(?:${GRANT})(\\.html)`, "g");
const GRANT_IN_FRAME = new RegExp(`(/frame/[a-z0-9-]+/)(?:${GRANT})`, "g");

/** Engine error messages can quote document URLs; grants must not reach flows */
export function redactGrants(text: string): string {
	return text
		.replace(GRANT_IN_DOCUMENT, "$1{grant}$2")
		.replace(GRANT_IN_FRAME, "$1{grant}");
}
