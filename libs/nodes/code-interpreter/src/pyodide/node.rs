...
impl NodeLogic for PythonInterpreterNode {
    ...
    async fn run(&self, context: &mut ExecutionContext) -> flow_like_types::Result<()> {
        flow_like_catalog_core::run_with_execute_gate!(context, { self.do_run(context).await })
    }
}
...
impl PythonInterpreterNode {
    async fn do_run(&self, context: &mut ExecutionContext) -> flow_like_types::Result<()> {
        // ── Read inputs ───────────────────────────────────────────────────
        ...
        let timeout_secs: f64 = context.evaluate_pin("timeout_secs").await.unwrap_or(30.0);

        let max_memory_mb: f64 = context.evaluate_pin("max_memory_mb").await.unwrap_or(256.0);
        ...
        if let Some(ref err) = response.error {
            ...
            context.log_message(&format!("Python execution failed: {err}"), LogLevel::Error);
        }
