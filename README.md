...
### Powered by Atlas Cloud (OpenAI-compatible) when you want a managed backend

<p align="center">
  <a href="https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=flow-like">
    <img src="assets/atlas-cloud-logo.png" alt="Atlas Cloud" width="200">
  </a>
</p>

> 🎁 **[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=flow-like)** is a full-modal, OpenAI-compatible AI inference platform. Flow-Like is local-first by design — but when you'd rather call a managed cloud than self-host, the built-in **Atlas Cloud Model** provider node connects you to DeepSeek, Qwen, GLM, Kimi, MiniMax and more through a single API, with the same execution tracing and guardrails as any other model. Budget-friendly plans: [coding plan](https://www.atlascloud.ai/console/coding-plan).

Drop an **Atlas Cloud Model** node into a flow (under `AI/Generative/Provider`), or configure it via OpenAI-compatible settings:
...
`deepseek-ai/deepseek-v4-pro` is a reasoning model — give it enough `max_tokens` (>= 512), otherwise the reply may be empty with `finish_reason=length`.
