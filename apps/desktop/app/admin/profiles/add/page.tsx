...
export default function AddProfilePage() {
	...
	const handleSave = useCallback(async () => {
		...
		try {
			...
			toast.success(
				isEditing ? t('profileTemplateUpdated', 'Profile template updated') : t('profileTemplateCreated', 'Profile template created'),
			);
			if (isEditing) {
		...
		<div className="container mx-auto p-6 max-w-4xl">
			<div className="mb-6">
				<h1 className="text-3xl font-bold">
					{isEditing ? t('editProfileTemplate', 'Edit Profile Template') : t('createNewProfileTemplate', 'Create New Profile Template')}
				</h1>
				<p className="text-muted-foreground">
					{t('configureReusableProfileTemplatesIncludingDefaultBitsHubsAndMetadata', "Configure reusable profile templates, including default bits, hubs, and metadata.")}
				</p>
			</div>
			...
			<div className="flex justify-end gap-2">
				...
				<Button onClick={handleSave} className="flex items-center gap-2">
					...
					{isEditing ? t('updateTemplate', 'Update Template') : t('createTemplate', 'Create Template')}
				</Button>
			</div>
		</div>
	);
