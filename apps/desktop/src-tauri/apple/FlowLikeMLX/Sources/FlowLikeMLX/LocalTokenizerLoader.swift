...
struct FlowLikeLocalTokenizerLoader: TokenizerLoader {
    func load(from directory: URL) async throws -> any MLXLMCommon.Tokenizer {
        let upstream = try await Tokenizers.AutoTokenizer.from(modelFolder: directory)
        return FlowLikeTokenizerAdapter(upstream)
    }
}

private struct FlowLikeTokenizerAdapter: MLXLMCommon.Tokenizer {
