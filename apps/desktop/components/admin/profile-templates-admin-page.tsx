...
import { Pencil, Plus, RefreshCw, Search, Trash2, UserRound } from "lucide-react";
...
export function ProfileTemplatesAdminPage({
	...
	const templates = useQuery<ProfileTemplate[]>({
		...
		queryFn: async () => {
			...
			return backend.apiState.get<ProfileTemplate[]>(profile.data, "info/profiles");
		},
	...
	const handleDelete = useCallback(
		async (template: ProfileTemplate) => {
			...
			try {
				await backend.apiState.del(profile.data, `admin/profiles/${template.id}`);
		...
		<main className="flex h-full min-h-0 w-full grow flex-col overflow-hidden bg-background">
			...
			<div className="flex items-center justify-between gap-4">
				...
				<div className="flex items-center gap-2">
					...
					{manageMode && (
						<Button size="sm" onClick={() => router.push("/admin/profiles/add")}>
					...
					{templates.isLoading ? (
						<div className="space-y-2">
							{Array.from({ length: 6 }).map((_, index) => (
								<Skeleton key={`profile-template-skeleton-${index}`} className="h-12 w-full" />
							))}
						</div>
					) : (
						<Table>
											...
											<TableCell>
												<div className="flex flex-wrap gap-1">
													...
													{(template.tags?.length ?? 0) > 3 && (
														<Badge variant="outline">+{(template.tags?.length ?? 0) - 3}</Badge>
													)}
												</div>
											</TableCell>
												...
												<TableCell className="text-right">
													<div className="flex justify-end gap-2">
														<Button
															...
															size="sm"
															onClick={() =>
																router.push(`/admin/profiles/add?id=${encodeURIComponent(template.id)}`)
															}
														>
