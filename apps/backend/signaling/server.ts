...
function onSubMessage(raw: string, ch: string) {
  ...
  try {
    ...
    if (message?.type === "publish" && message.topic && message._origin !== NODE_ID) {
...
async function connectSub(attempt = 0): Promise<void> {
  ...
  subConnecting = true;
  try {
    if (subClient) { try { await (subClient as any).close?.(); } catch {} subClient = null; }
    ...
    if (attempt > 0) console.log(`[Redis] Subscriber reconnected (attempt ${attempt})`);
  } catch (err) {
    console.error(`[Redis] Subscriber connect failed (attempt ${attempt}):`, (err as Error).message);
    subClient = null;
    const delay = Math.min(200 * 2 ** attempt, 30_000);
    setTimeout(() => { subConnecting = false; connectSub(attempt + 1); }, delay);
    return;
  }
  subConnecting = false;
}
...
setInterval(async () => {
  ...
  try { await pub.publish(HB_CHANNEL, "1"); } catch (err) {
...
const server = serve<WSData>({
  ...
  fetch(req, s) {
    ...
    if (pathname === "/") {
      ...
      return ok ? undefined : new Response("Upgrade failed", { status: 426, headers: { "Upgrade": "websocket" } });
    }
    // simple health endpoint
    return new Response("okay", { status: 200, headers: { "Content-Type": "text/plain" } });
  },
  ...
  websocket: {
    ...
    async message(ws, data) {
      ...
      if ((typeof data === "string" ? data.length : (data as Buffer).byteLength) > MAX_MSG_BYTES) {
...
async function shutdown() {
  ...
  subStopped = true;
  try { await pub.close?.(); } catch {}
  try { await subClient?.close?.(); } catch {}
  try { await sync.close?.(); } catch {}
  process.exit(0);
}
