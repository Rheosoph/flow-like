...
function runTests() {
	...
	setTimeout(() => {
		const receivedLocal = clients
			.slice(1)
			.filter((c) =>
				c.messages.some(
					(m) => m.type === "publish" && m.data?.test === "local",
				),
			).length;

		if (receivedLocal === numClients - 1) {
			console.log(
				`   ✓ All ${numClients - 1} other clients received the message`,
			);
			results.localPublish = true;
			results.crossWorkerPublish = true;
		} else {
			console.log(
				`   ✗ Only ${receivedLocal}/${numClients - 1} clients received`,
			);
		}
		...
		if (lastMsg && lastMsg.clients === numClients) {
			...
			console.log(
				`   ✗ Client count incorrect: ${lastMsg?.clients} (expected ${numClients})`,
			);
		}
	}, 600);
	...
	setTimeout(() => {
		const receivedAfterUnsub = clients[0].messages.some(
			(m) => m.type === "publish" && m.data?.test === "after-unsub",
		);
