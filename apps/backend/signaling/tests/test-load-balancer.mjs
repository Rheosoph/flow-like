...
const CONFIG = {
  ...
  messageDelay: parseInt(process.env.MESSAGE_DELAY || '100'),
    ...
    try {
      ...
      if (msg.type === 'publish') {
        ...
        if (stats.messagesReceived <= 10) {
          console.log(`[Client ${this.id}] ← Received message ${msg.messageId?.slice(0, 8)} from origin ${msg._origin?.slice(0, 8) || 'unknown'}`)
        }
      } else if (msg.type === 'pong') {
        // Health check response
      }
    } catch (err) {
      console.error(`[Client ${this.id}] Failed to parse message:`, err.message)
    }
  }
...
function verifyMessageDelivery() {
  ...
  console.log(`Messages tracked in delivery matrix: ${messageDeliveryMatrix.size}`)
  ...
  console.log(`  Complete deliveries:  ${completeDeliveries}/${allSentMessages.size}`)
  console.log(`  Partial deliveries:   ${partialDeliveries}/${allSentMessages.size}`)
  console.log(`  Missed deliveries:    ${missedDeliveries}/${allSentMessages.size}`)
  ...
  if (missedMessages.length > 0 && missedMessages.length <= 10) {
    ...
    missedMessages.slice(0, 10).forEach(id => {
      ...
      console.log(`  ${id.slice(0, 8)}: received by ${recipients ? recipients.size : 0} clients`)
    })
  }
  ...
  console.log(`  Clients with perfect delivery: ${clientsWithPerfectDelivery}/${CONFIG.numClients}`)
  ...
  if (clientDeliveryIssues.length > 0 && clientDeliveryIssues.length <= 10) {
    ...
    clientDeliveryIssues.slice(0, 10).forEach(issue => {
      console.log(`  Client ${issue.clientId}: expected ${issue.expected}, got ${issue.actual}, missing ${issue.missing}`)
    })
  }
...
function generateReport() {
  ...
  console.log(`   Clients connected:    ${stats.connected}/${CONFIG.numClients}`)
  console.log(`   Clients subscribed:   ${stats.subscribed}/${CONFIG.numClients}`)
  ...
  const expectedMessages = CONFIG.numClients * CONFIG.messagesPerClient * (CONFIG.numClients - 1)
  const deliveryRate = ((stats.messagesReceived / expectedMessages) * 100).toFixed(2)
  ...
  if (stats.reportedClientCounts.length > 0) {
    const avgCount = stats.reportedClientCounts.reduce((a, b) => a + b, 0) / stats.reportedClientCounts.length
  ...
  if (stats.clientLatencies.length > 0) {
    const avgLatency = stats.clientLatencies.reduce((a, b) => a + b, 0) / stats.clientLatencies.length
    const minLatency = Math.min(...stats.clientLatencies)
    const maxLatency = Math.max(...stats.clientLatencies)
    const p95Latency = stats.clientLatencies.sort((a, b) => a - b)[Math.floor(stats.clientLatencies.length * 0.95)]
  ...
  console.log('\n📈 Per-Client Statistics:')
  const clientsReceivedAll = clients.filter(c => c.messagesReceived >= CONFIG.numClients * CONFIG.messagesPerClient - CONFIG.messagesPerClient).length
  console.log(`   Clients with full delivery: ${clientsReceivedAll}/${CONFIG.numClients}`)

  const clientsWithErrors = clients.filter(c => c.errors.length > 0).length
  console.log(`   Clients with errors:        ${clientsWithErrors}/${CONFIG.numClients}`)

  // Message delivery verification
  console.log('\n✅ Message Delivery Verification:')
  console.log(`   Total unique messages sent:     ${allSentMessages.size}`)
  console.log(`   Complete deliveries:            ${stats.completeDeliveries || 0}`)
  console.log(`   Partial deliveries:             ${stats.partialDeliveries || 0}`)
  console.log(`   Missed deliveries:              ${stats.missedDeliveries || 0}`)
  console.log(`   Clients with perfect delivery:  ${stats.clientsWithPerfectDelivery || 0}/${CONFIG.numClients}`)
  console.log(`   Clients with delivery issues:   ${stats.totalClientDeliveryIssues || 0}/${CONFIG.numClients}`)

  const messageDeliveryRate = allSentMessages.size > 0
    ? ((stats.completeDeliveries || 0) / allSentMessages.size * 100).toFixed(2)
    : '0.00'
  ...
  const successCriteria = {
    ...
    deliveryRateGood: parseFloat(deliveryRate) >= 95,
    messageDeliveryPerfect: parseFloat(messageDeliveryRate) >= 95,
    allClientsReceivedAll: (stats.clientsWithPerfectDelivery || 0) >= CONFIG.numClients * 0.95, // 95% of clients
  ...
  if (allPassed) {
    ...
    console.log('\nFailed criteria:')
    if (!successCriteria.allConnected) console.log('  - Not all clients connected')
    if (!successCriteria.allSubscribed) console.log('  - Not all clients subscribed')
    if (!successCriteria.deliveryRateGood) console.log('  - Message delivery rate below 95%')
    if (!successCriteria.messageDeliveryPerfect) console.log('  - Message delivery verification below 95%')
    if (!successCriteria.allClientsReceivedAll) console.log('  - Not all clients received all messages')
    if (!successCriteria.multipleNodes) console.log('  - No server nodes detected')
    if (!successCriteria.lowErrors) console.log('  - Too many errors encountered')
    testFailed = true
  }
  ...
  if (stats.uniqueOrigins.size === 1) {
    ...
    console.log('   For true load balancer testing, run multiple server instances:')
    console.log('   PORT=4444 NODE_ID=node1 bun run server.ts')
    console.log('   PORT=4445 NODE_ID=node2 bun run server.ts')
    console.log('   Then configure your load balancer to distribute across them.\n')
  }
  ...
  if (parseFloat(messageDeliveryRate) < 95) {
    ...
    console.log('   - Verify all server instances are subscribed to the same channel\n')
  }
}
