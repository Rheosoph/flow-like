...
Those session policies also carry the KMS grant a bucket on a customer-managed
key needs, so the runtime role must allow `kms:Decrypt` and
`kms:GenerateDataKey` on that key. Set `S3_KMS_KEY_ARN` (or the per-bucket
`META_BUCKET_KMS_KEY_ARN`, `CONTENT_BUCKET_KMS_KEY_ARN` and
`LOG_BUCKET_KMS_KEY_ARN`) to narrow the grant to named keys and to send explicit
SSE-KMS headers on writes. See
