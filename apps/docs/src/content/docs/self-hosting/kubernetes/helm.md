...
| --- | --- |
| AWS | `AWS_REGION`, `META_BUCKET`, `CONTENT_BUCKET`, `LOG_BUCKET`, `RUNTIME_ROLE_ARN`; either static `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` or workload identity, plus optional `AWS_ENDPOINT`, `AWS_USE_PATH_STYLE`, and `S3_KMS_KEY_ARN` for buckets on a customer-managed KMS key |
