...
description: Build a deterministic incident triage Flow, inspect its node graph, edit it in FlowScript and Studio, trace failures, and save a tested version.
seo:
  title: "Flow-Like FlowScript Tutorial: Build an Incident Triage Workflow"
  topics:
    - FlowScript tutorial
...
<WorkflowFigure
  src={triageFull}
  alt="Six-node Incident Triage workflow with Branch routing to Log Error and Print Info while normalized report data feeds the condition and both messages."
  caption="The complete six-node FlowScript rendered in Studio. Solid dark wires carry execution; colored dotted wires carry typed data."
/>
