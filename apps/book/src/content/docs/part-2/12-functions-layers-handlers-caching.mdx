...
```text
Event ──▶ impure operation ──▶ impure operation
              ▲
              │ needs an input
          pure formatter
              ▲
...
```ts
const resolution = normalized.resolveSystem(directoryRevision)
```
...
```ts
@cache({ namespace: "incident-system-directory-v1", ttlSeconds: 600, scope: "app" })
function resolveSystem(
    systemId: string,
    directoryRevision: string,
): (team: string, runbook: string) {
...
those surfaces converge, this book always writes the complete object form. `ttlSeconds: 0` means no
expiry and should be reserved for entries with an explicit invalidation and cleanup plan.
...
```text
commit durable update
    └─ advance its revision
        └─ invalidate the matching cache namespace
            └─ publish success
```
