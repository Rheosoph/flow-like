...
Logic reads like code, too. Here's a real function from a test board — a for-loop over a `controlForEach` node, registering database tables into a query session:
...
record.folders = listFolders.folders   // desugars to another structSet on apply
```

That `record.folders = …` line is real FlowScript. On apply it expands back into a `structSet` node and rebinds the variable — so the text stays clean while the graph stays correct.
