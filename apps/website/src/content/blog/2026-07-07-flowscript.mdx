...
Node outputs are destructured by pin name (`const { session } = …`, `const { rows } = …`), or read as field access on the binding (`result.rows`). Branching nodes open a block per execution pin. And there's syntactic sugar for the thing everyone does constantly — building up a struct field by field:
