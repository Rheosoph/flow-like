# Flow-Like Widget Template (Svelte)
...
Svelte needs no SDK adapter — the nanostores exposed by the bridge implement
the Svelte store contract natively, so `bridge.$props` can be read with the
`$store` auto-subscription syntax directly in markup.
...
module (here: `index.ts` mounting `Widget.svelte`). Style with the host theme
