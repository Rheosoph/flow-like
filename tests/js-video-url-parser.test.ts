...
describe("js-video-url-parser shim", () => {
	test("parses the video providers used by PlateJS media embeds", () => {
		...
		expect(videoParser.parse("https://player.vimeo.com/video/123456789")).toEqual(
			{
				provider: "vimeo",
				id: "123456789",
			},
		);
		expect(videoParser.parse("https://www.dailymotion.com/video/x7tgad0")).toEqual(
			{
				provider: "dailymotion",
				id: "x7tgad0",
			},
		);
		expect(videoParser.parse("https://player.youku.com/embed/XNDQ4NDYxMzM2OA==")).toEqual(
			{
				provider: "youku",
				id: "XNDQ4NDYxMzM2OA==",
			},
	...
	test("rejects invalid and oversized input", () => {
		...
		expect(videoParser.parse(`https://youtu.be/${"a".repeat(2049)}`)).toBeUndefined();
	});
});
